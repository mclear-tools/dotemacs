...
#+begin_src sh
brew install emacs-plus@28 --with-no-frame-refocus --with-xwidgets --with-nobu417-big-sur-icon --with-native-comp
#+end_src
...
the latest release being best (I'm currently on emacs 28).
